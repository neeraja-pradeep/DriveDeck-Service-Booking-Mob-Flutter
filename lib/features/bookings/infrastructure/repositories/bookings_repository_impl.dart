...
class BookingsRepositoryImpl implements BookingsRepository {
  ...
  Future<Either<Failure, CachedData<BookingsList>?>> getBookings({
    ...
    try {
      ...
      return Left(BookingFetchFailure(e.toString()));
    }
  }
  ...
  Future<Either<Failure, Booking>> getBookingDetails(String bookingId) async {
    try {
      ...
      final failure = NetworkExceptions.getDioException(e);
  ...
  Future<Either<Failure, Booking>> cancelBooking(String bookingId) async {
    try {
      ...
      // Update local cache
      await localDataSource.updateBookingInCache(
        bookingId,
        {
      ...
      if (e.response?.statusCode == 400) {
        ...
        if (message?.contains('already cancelled') ?? false) {
          return const Left(BookingAlreadyCancelledFailure());
        }
        return Left(CancellationNotAllowedFailure(message));
      }
