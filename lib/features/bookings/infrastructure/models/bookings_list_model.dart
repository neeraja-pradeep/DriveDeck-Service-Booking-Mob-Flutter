...
class BookingsListModel {
  ...
  factory BookingsListModel.fromJson(dynamic json) {
    ...
    return const BookingsListModel(upcomingBookings: [], pastBookings: []);
  }
  ...
  static BookingsListModel _fromFlatList(List<dynamic> json) {
    ...
    final upcoming = allBookings.where((b) => b.status.isUpcoming).toList()
      ..sort((a, b) => a.bookingDate.compareTo(b.bookingDate));

    final past = allBookings.where((b) => b.status.isPast).toList()
      ..sort((a, b) => b.bookingDate.compareTo(a.bookingDate));

    return BookingsListModel(upcomingBookings: upcoming, pastBookings: past);
  }
  ...
  factory BookingsListModel.fromDomain(BookingsList entity) {
    return BookingsListModel(
      upcomingBookings: entity.upcomingBookings
          .map((b) => BookingModel.fromDomain(b))
          .toList(),
      pastBookings: entity.pastBookings
          .map((b) => BookingModel.fromDomain(b))
          .toList(),
    );
  }
